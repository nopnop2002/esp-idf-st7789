...
typedef struct {
	...
	int	imageWidth;			// Width of the image file
	int	imageHeight;		// Height of the image file
} JpegDev;
...
static UINT outfunc(JDEC *decoder, void *bitmap, JRECT *rect) {
	...
	if (jd->imageWidth < rect->right) jd->imageWidth = rect->right;
	if (jd->imageHeight < rect->bottom) jd->imageHeight = rect->bottom;
...
esp_err_t decode_image(pixel_s ***pixels, char * file, uint16_t width, uint16_t height, uint16_t * imageWidth, uint16_t * imageHeight) {
	...
	*pixels = calloc(height, sizeof(pixel_s *));
	...
	ESP_LOGD(TAG, "imageWidth=%d imageHeight=%d", jd.imageWidth, jd.imageHeight);
	*imageWidth = jd.imageWidth;
	*imageHeight = jd.imageHeight;
	free(work);
	fclose(jd.fp);
	return ret;
	err:
